...
namespace SPladisonsYoyoMod.Content.Items.Placeables
{
    ...
    public class SpaceChestTile : PladTile
    {
		...
		{
			Tile tile = Main.tile[i, j];
			if (tile != null && (tile.frameX == 0 || tile.frameX == 18 * 2) && tile.frameY == 0)
			{
				spriteBatch.End();
				...
				if (chest > 0)
				{
					...
					var position = new Vector2(i * 16 + Main.LocalPlayer.position.X * 0.1f - Main.screenPosition.X, j * 16 + Main.LocalPlayer.position.Y * 0.1f - Main.screenPosition.Y) + zero;
					shader.Apply(null, new DrawData(texture, position, new Rectangle((int)(tile.frameX / 36) * 36, Main.chest[chest].frame * 38, 18, 18), Color.White));

					spriteBatch.Draw(texture, new Vector2(i * 16 - (int)Main.screenPosition.X, j * 16 - (int)Main.screenPosition.Y) + zero, new Rectangle(tile.frameX, tile.frameY + Main.chest[chest].frame * 38, 16, height), Color.White, 0f, Vector2.Zero, 1f, SpriteEffects.None, 0f);
				}
