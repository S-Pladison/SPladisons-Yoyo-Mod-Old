...
namespace SPladisonsYoyoMod.Content.Items.Weapons
{
    public class BloomingDeath : YoyoItem
    {
        ...
        public override void YoyoSetStaticDefaults()
        {
            ...
            this.SetTooltip(eng: "Yoyo and string poison enemies",
                            rus: "Йо-йо и нить отравляют врагов");
        }
    ...
    public class BloomingDeathProjectile : YoyoProjectile
    {
        ...
        {
            StringColorMethodInfo = typeof(Main).GetMethod("TryApplyingPlayerStringColor", BindingFlags.NonPublic | BindingFlags.Static);
            StringTexture = ModContent.GetTexture("SPladisonsYoyoMod/Assets/Textures/Misc/Extra_9");
            LeafTexture = ModContent.GetTexture("SPladisonsYoyoMod/Assets/Textures/Misc/Extra_10");
        }
        ...
        public override void OnSpawn()
        {
            ...
            trail.SetEffectTexture(ModContent.GetTexture("SPladisonsYoyoMod/Assets/Textures/Misc/Extra_11").Value);
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            var texture = ModContent.GetTexture("SPladisonsYoyoMod/Assets/Textures/Misc/Extra_12").Value;
            var color = Lighting.GetColor((int)Projectile.Center.X / 16, (int)(Projectile.Center.Y / 16f), Color.White);

            for (int i = 0; i < 5; i++) Main.EntitySpriteDraw(texture, drawPosition, null, color * 0.22f, Projectile.rotation * 0.05f + MathHelper.Pi + (MathHelper.TwoPi / 5 * i), new Vector2(9, 20), 1.22f, SpriteEffects.None, 0);
            for (int i = 0; i < 5; i++) Main.EntitySpriteDraw(texture, drawPosition, null, color * 0.88f, Projectile.rotation * 0.05f + (MathHelper.TwoPi / 5 * i), new Vector2(9, 20), 1f, SpriteEffects.None, 0);

            return true;
        }

        public static void DrawCustomString(Projectile projectile, Vector2 mountedCenter, int key)
        {
            ...
            {
                int num5 = -1;
                if (projectile.position.X + (float)(projectile.width / 2) < Main.player[projectile.owner].position.X + (float)(Main.player[projectile.owner].width / 2)) num5 = 1;
                num5 *= -1;
            ...
            while (flag)
            {
                ...
                vector.X += num2;
                vector.Y += num3;
                num2 = projectile.position.X + (float)projectile.width * 0.5f - vector.X - offset.X;
                num3 = projectile.position.Y + (float)projectile.height * 0.1f - vector.Y - offset.Y;
                ...
                // Changing string texture frame
                int frame = 2;
                {
                    if (counter <= key) frame = 0;
                    else if (counter <= (key + 1)) frame = 1;
                }
                counter++;
                ...
                Color color = Color.White;
                {
                    ...
                    {
                        // ...
                        if (frame == 0) color = (Color)(StringColorMethodInfo?.Invoke(null, new object[] { Main.player[projectile.owner].stringColor, Color.White }) ?? Color.White);
                    }
                    catch { }
                    color = Lighting.GetColor((int)vector.X / 16, (int)(vector.Y / 16f), color);
                    if (frame == 0) color.A = (byte)(color.A * 0.4f);
                }
